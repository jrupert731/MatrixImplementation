...
bool run_checks(int &n, double *&a, double *&v)
{
  ...
  if (answer && debugging)
    cout << "Input Basic Checks Passed." << endl;
  return answer;
}
...
bool row_is_zeros(const double *row, const int &n)
{
  ...
  {
    contains_only_zeros = contains_only_zeros && (row[j] > -1e-14 && row[j] < 1e-14);
  }
...
{
  int j = i + 1;
  while ((m[get_index(n, j, i)] > -1e-14 && m[get_index(n, j, i)] < 1e-14) && j < n)
    j++;
    ...
    for (int j = i + 1; j < n; j++)
    {
      ...
      if (below_pivot != 0 && pivot != 0) // don't need fix because
      {
        ...
        for (int k = i; k < n; k++)
        {
          ...
          if (rescaled < 1e-14 && rescaled > -1e-14)
            rescaled = 0;
  ...
  for (int i = n - 1; i >= 0; i--)
  {
    if (!row_is_zeros(i * n + m, n))
    {
      if (num_vars_solved != 0)
      {
        ...
        int solving_index = 0;
        while ((m[get_index(n, i, solving_index)] > -1e-14 && m[get_index(n, i, solving_index)] < 1e-14) && solving_index < n)
          solving_index += 1;
